...
The system SHALL provide `sysi init` to initialize a repo-local sysi-orchestrated monorepo.

#### Scenario: Initialize empty repository
- **WHEN** a user runs `sysi init` at the repository root
- **THEN** the system creates `.sysi/`, scaffolds `/system`, records the phase as `design`, and prints the next available commands
...
- **THEN** the system preserves existing state and reports that the repository is already initialized
